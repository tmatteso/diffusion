...
# the distogram is a N x N x bin_num matrix where each entry is a 1 if a residue distance is in the right num_bin bucket
# the point of the distogram is to keep learned features geometrically grounded. otherwise we can get weird output coords like your VAE
class Distogram(nn.Module):
    ...
    def forward(
        ...
        coords_mask: Optional[Bool[torch.Tensor, "... total_atom_count"]] = None,
    ) -> tuple[
        ...
        Returns:
            f_distogram: (..., total_atom_count, total_atom_count, n_bins [+1]) — one-hot bin assignment;
        ...
        diff = (rearrange(coords, "... n d -> ... n 1 d")
              - rearrange(coords, "... n d -> ... 1 n d"))
        dist = diff.norm(dim=-1)

        # ---- 2. Bin assignment & one-hot (..., total_atom_count, total_atom_count, n_bins[+1]) ----
        ...
        f_pair_mask = rearrange(atom_valid, "... n -> ... n 1") & rearrange(atom_valid, "... n -> ... 1 n")
...
def featurize_batch(
    ...
    items: list[dict] = []
    for ix in range(B):
        atom37_positions: Float[torch.Tensor, "N_res 37 3"] = batch.atom_positions[ix].to(device)
        atom37_mask:      Float[torch.Tensor, "N_res 37"]   = batch.atom_mask[ix].to(device)
        index:            Float[torch.Tensor, "N_res"]      = batch.residue_index[ix].to(device)
        aa_sequence: str = batch.seq[ix]
        ...
        atom5_msk    = rearrange(atom5_msk, "1 n a -> n a")      # (N_res_i, 5)
    ...
    gt_res_distogram   = torch.stack([it["gt_res_distogram"]  for it in items])  # (B, N_res, N_res, n_templ_bins)
    ...
    _tok_single:    Int[torch.Tensor, "N_atom"] = (
        torch.arange(N_res_total, dtype=torch.long, device=device).repeat_interleave(Natom)
    )
