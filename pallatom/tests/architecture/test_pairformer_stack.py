...
def test_pairformer_block_output_shape(block, v):
    with torch.no_grad():
        out = block(v)
    assert out.shape == (B, N_RES, N_RES, C)


def test_pairformer_block_output_finite(block, v):
    with torch.no_grad():
        out = block(v)
    assert torch.isfinite(out).all()


def test_pairformer_block_output_dtype(block, v):
    with torch.no_grad():
        out = block(v)
    assert out.dtype == v.dtype


def test_pairformer_block_changes_input(block, v):
    with torch.no_grad():
        out = block(v)
    assert not torch.allclose(out, v)


def test_pairformer_block_gradient_flows(block, v):
...
def test_pairformer_block_row_attn_preserves_col_symmetry(block, v):
...
def test_pairformer_block_different_batch_items_independent(block):
...
def test_pairformer_stack_output_shape(stack, v):
    with torch.no_grad():
        out = stack(v)
    assert out.shape == (B, N_RES, N_RES, C)


def test_pairformer_stack_output_finite(stack, v):
    with torch.no_grad():
        out = stack(v)
    assert torch.isfinite(out).all()


def test_pairformer_stack_block_count():
    s = PairformerStack(C, n_blocks=5, n_heads=N_HEADS)
    assert len(s.blocks) == 5


def test_pairformer_stack_depth_changes_output():
...
def test_pairformer_stack_gradient_flows(stack, v):
    v_g = v.clone().requires_grad_(True)
    out = stack(v_g)
    reduce(out, "b n m c -> ", "sum").backward()
    assert v_g.grad is not None
