...
def ca() -> Float[torch.Tensor, "B N_res 3"]:
    """Provide random Cα atom positions (B, N_RES, 3)."""
    return torch.randn(B, N_RES, 3)
...
def test_pseudo_cb_output_not_equal_ca(n, ca, c):
    """pseudo_cb coordinates differ from Cα coordinates."""
    out = pseudo_cb(n, ca, c)
...
def test_protein_is_frozen(valid_protein):
    """Protein raises an exception when a field is reassigned after construction."""
...
def test_protein_rejects_float_aatype():
    """Protein raises when aatype is float32 instead of an integer dtype."""
    with pytest.raises(TypeCheckError):
        Protein(
            atom_positions=np.zeros((N_RES, N_ATOM_TYPE, 3), dtype=np.float32),
...
@pytest.fixture
def coords_dict() -> Mapping[str, np.ndarray]:
...
@pytest.fixture
def np_example(coords_dict: Mapping[str, np.ndarray]) -> Mapping[str, np.ndarray]:
...
@pytest.fixture
def short_np_example() -> Mapping[str, np.ndarray]:
...
@pytest.fixture
def long_np_example() -> Mapping[str, np.ndarray]:
...
@pytest.fixture
def full_mask_np_example() -> Mapping[str, np.ndarray]:
...
@pytest.fixture
def ca_only_np_example() -> Mapping[str, np.ndarray]:
